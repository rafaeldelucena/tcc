...
Geralmente tais redes possuem centenas ou milhares de n\'os sensores e possuem as seguintes caracter\'isticas: pouca mem\'oria, pouco alcance do r\'adio, baixa capacidade de processamento e bateria, e custo reduzido. A figura \ref{wsnOverview} d\'a uma vis\~ao geral de uma rede de sensores sem fio e a comunica\c{c}\~ao com outros sistemas.

Comunicam-se entre si e com esta\c{c}\~oes base utilizando seus r\'adios sem fio.  Desta forma permite dissemina\c{c}\~ao da informa\c{c}\~ao para processamento remoto, visualiza\c{c}\~ao, an\'alise e armazenamento. 
...
\subsection{N\'os Sincronizadores}

\'E muito comum nodos destas redes utilizarem um espectro diferente de comunica\c{c}\~ao, necessitando uma integra\c{c}\~ao com a rede sem fio trandicional 802.11. Uma forma de integrar os n\'os sensores \'e utilizar n\'os espec\'ificos para executarem este trabalho.

Alguns n\'os da rede s\~ao utilizados como esta\c{c}\~oes de sincroniza\c{c}\~ao, que fucionam como pontes para diferentes meios de comunica\c{c}\~ao.
...
Clientes que utililiza tais servi\c{c}os podem ser outras solu\c{c}\~oes, aplica\c{c}\~oes, processos ou usu\'arios.

%\section{REST}

\section{Servi\c{c}o WEB}
...
RESTFul \'e uma maneira de aplicar os princ\'ipios de design REST para servi\c{c}os web. Uma restri\c{c}\~ao importante imposta pelo REST \'e o comportamento \textit{stateless} do servidor, que n\~ao necessita possuir nenhuma informa\c{c}\~ao adicional al\'em dos dados contidos nas requisi\c{c}\~oes para tratar e responder de acordo para o cliente.

Estas requisi\c{c}\~oes geradas por clientes distintos devem conter todas as informa\c{c}\~oes necess\'arias para que sejam executadas e respondidas por servidores.
...
A comunica\c{c}\~ao entre os pontos no CoAP \'e ass\'incrona usando o UDP. A confiabilidade \'e um par\^ametro opcional e funciona atrav\'es de um mecanismo de retransmiss\~ao exponencial. Possui 4 tipos de mensagem: Confirm\'avel, N\~ao-Confirm\'avel, Confirma\c{c}\~ao (ACK) e Reset.

\subsection{Formato das mensagens}
Uma mensagem CoAP deve caber num \'unico pacote IP, para que seja transmitida numa camada de enlace limitada. A figura \ref{coapFormat} mostra o formato do pacote.
...
Possui uma su\'ite de testes para regress\~ao, utilizando o \textit{framework} de testes CUnit (http://cunit.sourceforge.net/). A documenta\c{c}\~ao pode ser encontrada em \cite{libcoap}.
...
Tamb\'em \'e poss\'ivel montar os pacotes CoAP a partir de uma sequ\^encia de caracteres recebidos de uma placa de rede. Abaixo exemplos de uso da biblioteca retirados de \cite{cantcoap}.
