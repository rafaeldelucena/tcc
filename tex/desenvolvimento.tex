...
No mecanismo de requisic\c{c}\~o e resposta, as requisi\c{c}\~oes pendentes foram armezenadas num Hash com a chave sendo o token gerado pelo cliente. O protocolo CoAP foi modelado conforme \'e mostrado na figura \ref{uml} abaixo:
\begin{figure}[H]
   \label{uml}
   \centering
...
Foi desenvolvimento de um mecanismo de retransmiss\~ao de mensagens n\~ao-confirmadas utilizando uma lista ordenada e alarme configurado para o pr\'oximo reenvio. A figura \ref{diagramaSequenciaCoapClient} ilustra um cen\'ario de recebimento de mensagem CoAP e envio via GPRS do gateway e a intera\c{c}\~ao entre os m\'odulos em software. 

\begin{figure}[H]
   \label{diagramaSequenciaCoapClient}
   \centering
   \includegraphics[width=1.0\textwidth]{figuras/diagramaSequenciaCoapClient.png}
   \caption{Diagrama de sequ\^encia de um fluxo comum da aplica\c{c}\~ao gateway.}
\end{figure}
